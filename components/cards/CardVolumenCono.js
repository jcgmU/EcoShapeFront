...
export default function CardVolumenCono() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost:5001/api/volumen-cono", {
        method: "POST",
        headers: {
    ...
    <div className="w-full bg-[#f2f3f7] rounded-xl transition-all duration-200 ease-in-out shadow-[1em_1em_1em_#d8dae0b1,-0.75em_-0.75em_1em_#ffffff] border-[1.5px] border-[#f2f3f7] hover:bg-[#d3ddf1]">
      <div className="m-5 flex flex-col gap-3">
        <div className="flex flex-col gap-1 text-gray-800">
          <h2 className="text-xl font-bold mb-2">Volumen Cono</h2>
          <p className="text-gray-600 mb-4">
            Calcula el volumen de la pared de un cono hueco a partir de sus
            dimensiones.
          </p>
        </div>
          ...
          <div className="mt-4">
            ...
            {result && (
              <div className="mt-4 p-4 bg-white rounded-lg">
                ...
                <p className="text-sm">
                  <strong>Volumen de la pared del cono:</strong>{" "}
                  {result.volumen} cm³
                </p>
              </div>
            )}
